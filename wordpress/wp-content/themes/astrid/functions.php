...
function astrid_branding() {
	...
	if ( function_exists( 'the_custom_logo' ) && has_custom_logo() ) {
		...
		$html = sprintf(
			'<a href="%1$s" class="custom-logo-link" rel="home"><p class="site-title"><a href="' . esc_url( home_url( '/' ) ) . '" rel="home">' . esc_html(get_bloginfo('name')) . '</a></p><p class="site-description">' . esc_html(get_bloginfo( 'description' )) . '</p>%2$s</a>',
			esc_url( home_url( '/' ) ),
			wp_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr )
		);
	    echo $html;
	} elseif ( $site_logo ) {
		echo '<a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr(get_bloginfo('name')) . '"><img class="site-logo" src="' . esc_url($site_logo) . '" alt="' . esc_attr(get_bloginfo('name')) . '" /><p class="site-title"><a href="' . esc_url( home_url( '/' ) ) . '" rel="home">' . esc_html(get_bloginfo('name')) . '</a></p></a>'; 
	} else {
